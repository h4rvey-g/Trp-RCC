run_risk_DEG_tumor <- function(data_filt, data_risk_score) {
    ...
    write_tsv(
        ...
        "data/106.risk_score_DEG/data_dds_tumor_only.tsv"
    )
    data_dds
}

run_risk_enrich_tumor <- function(data_dds, msigdb) {
    data_dds <- data_dds %>%
        as_tibble() %>%
    ...
    ggsave("result/107.risk_score_enrich/gsea_metabolism_tumor_only.png", p)
    p <- plotGSEA(gsea_metabolism, plot_type = "bar", colour = c("navyblue", "orange"))
    ggsave("result/107.risk_score_enrich/gsea_metabolism_tumor_only_bar.png", p)
    write_tsv(gsea_metabolism$gsea_df, "data/107.risk_score_enrich/gsea_metabolism_tumor_only.tsv")
